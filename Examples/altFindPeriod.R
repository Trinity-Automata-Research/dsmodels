...
#helper for unique bucket
#in 1d could sort first then find in one pass.
#in general, need n! comparisons
numBuckets=function(candidates, epsilon=sqrt(.Machine$double.eps)){ #removes all elements close to the first, repeats
  bucket=0
  while(length(candidates)>0){
    test=candidates[1]
    dists=mapply(sqdist,test,candidates)
    candidates[dists < epsilon]=NULL
    bucket=bucket+1
  }
  bucket
}
...
#~~~~~~~~~~~~~~~~~~~~~~~~
#classical, whole period. if default is null, is unused
find.period = function( x, y, iters=NULL, maxPeriod=128, initIters=500, numTries=NULL, powerOf2=TRUE,
                        epsilon=sqrt(.Machine$double.eps), convergeCheck=NULL, ...){
  #moves all the points. stops if they are either all infinite, fixed, or if(crop==TRUE), outside of range
  ...
  return(wholePeriodCheck(candidates,epsilon,maxPeriod,powerOf2)) #any period 128 or greater is called 128
  return(Inf)
}


#~~~~~~~~~~~~~~~~~~~~~~~~
#dsmodels current- convergeCheck, wholePeriod. if default is null, is unused
...
find.period = function( x, y, iters=NULL, maxPeriod=128, initIters=100, numTries=5, powerOf2=TRUE,
  ...
  for(i in 1:numTries) {
    ...
    convergePoint <- apply(candidates[[maxPeriod*2]]$x, candidates[[maxPeriod*2]]$y,...,iters=1,accumulate=FALSE)
    compareCandidates= apply(convergePoint$x, convergePoint$y, ...,iters=maxPeriod*2-1,accumulate=TRUE)
    last=compareCandidates[[maxPeriod*2]]
...
#check 1:2^n against 2^n+1:2*2^n
wholePeriodCheck=function(candidates,epsilon,maxPeriod,powerOf2){
  period=FALSE
  j=1
  while(j<=maxPeriod && !period){ #check for fixed or periodicity
    test=candidates[1:j]
...
#~~~~~~~~~~~~~~~~~~~~~~~~
#convergeCheck, unique buckets. if default is null, is unused
#for each pass, prints true or false for if it converged
find.period = function( x, y, iters=NULL, maxPeriod=128, initIters=100, numTries=5, powerOf2=TRUE,
                        epsilon=sqrt(.Machine$double.eps), convergeCheck=NULL, ...){
  #moves all the points. stops if they are either all infinite, fixed, or if(crop==TRUE), outside of range
